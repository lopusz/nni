...
===============================
This tutorial optimizes the model in `official Pytorch quickstart` with auto-tuning.
...
# *linear_width*, *activation_type*, *learning_rate*, and *momentum*.
#
# Assuming we have following prior knowledge for these hyperparameters:
#
#  1. *linear_width* should be one of 128, 256, 512.
#  2. *activation_type* should be one of 'relu', 'relu6', 'tanh'.
#  3. *learning_rate* should be a float between 0.0001 and 0.1, and it follows exponential distribution.
#  4. *momentum* should be a float between 0.85 and 0.95.
#
# Then similar to ``numpy.random``,
# in NNI we call *linear_width* and *activation_type* ``choice``s;
# *learning_rate* is ``loguniform``;
# and *momentum* is ``uniform``.
#
...
search_space = {
    'linear_width': {'_type': 'choice', '_value': [128, 256, 512]},
    'activation_type': {'_type': 'choice', '_value': ['relu', 'tanh', 'swish', None]},
    'learning_rate': {'_type': 'loguniform', '_value': [0.0001, 0.1]},
    'momentum': {'_type': 'uniform', '_value': [0.85, 0.95]},
}
